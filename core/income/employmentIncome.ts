import { EMPLOYMENT_INCOME_DEDUCTION_2025, type DeductionBracket } from '@data/taxTables/2025'
...
export function employmentIncomeDeduction(
  salaryIncome: number,
  brackets: readonly DeductionBracket[] = EMPLOYMENT_INCOME_DEDUCTION_2025,
): number {
  if (salaryIncome <= 0) return 0
  const bracket = brackets.find((b) => b.upTo === null || salaryIncome <= b.upTo)
...
export function employmentIncome(
  ...
  return Math.max(0, salaryIncome - employmentIncomeDeduction(salaryIncome, brackets))
}
